...
import './unSuitableCandidates.css'

class UnSuitableCandidates extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    .then((response) => {
      ...
      .then((json) => {
        ...
        this.setState({ candidates : json.ReturnObject.NotSuitableCandidates })
      });
  }


  renderCandidates() {
    let notSuitable = this.state.candidates.map((candidate) => {
      return (
      ...
      <div >
        {notSuitable}
          <ListGroupItem style={{paddingLeft: '8px'}}>
            <button className="ActionBtn">Mark Candidate as Not Suitable</button>
          </ListGroupItem>
      </div>
    )
  }
  ...
  render() {
    ...
    return (
      <Table>
        <ListGroup>
          <ListGroupItem bsStyle="danger" style={{ borderRadius: '0px'}}>
            <span className="glyphicon glyphicon-minus collapseBar"></span>
            Not Suitable ( {this.state.candidates.length} )
          </ListGroupItem>
...
export default UnSuitableCandidates;